...
const rootReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case FILTER_BY_CATEGORY:
      const categoryFilter = action.payload === 'All' ? state.products :
    ...
    case FILTER_BY_BRAND:
      const brandFilter = action.payload === 'All' ? state.products :
        state.products.filter(el => el.brand.map(el => el).includes(action.payload))
      return {
        ...state,
